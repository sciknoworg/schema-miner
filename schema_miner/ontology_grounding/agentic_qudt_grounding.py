...
def _load_qudt_ontology(url: str, return_vecStore: bool = True) -> str | FAISS:
    ...
    if not return_vecStore: return ontology
...
def agentic_qudt_grounding(llm_model_name: str, process_schema: dict | Path, result_file_path: str, save_schema: bool = False) -> dict | None:
    ...
    def ground_qudt_entity(query: str):
        ...
        var_dict = {'quantityKind': relevantQuantityKind, 'units': relevantQudtUnits, 'process_name': ProcessConfig.Process_name, 'process_description': ProcessConfig.Process_description, 'query': query}
    ...
    llm = ChatOpenAI(model = llm_model_name, temperature = 0, model_kwargs={'response_format': {'type': 'json_object'}})
    ...
    for key in schema_properties_desc:
        # Query agent with schema property
        agent_response = agent_executor.invoke({'query': f'{' - '.join(key)}'})
        agent_response = json.loads(agent_response['output'])
        
        # Skip if grounding is invalid
        if not agent_response['valid']: continue
        
        # Inject ontology into schema
        _inject_quantity_schema(schema, key[0].split(' '), ontology['properties'], agent_response)
