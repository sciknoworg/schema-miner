...
if __name__ == "__main__":
    ...
    results_file_path = "../results/test/stage-1/Atomic-Layer-Deposition/experimental-schema"
    ...
    if not process_specification:
        raise ValueError("PDF text extraction failed for process specification document. Cannot continue to schema extraction.")
    schema = extract_schema_stage1(llm_model_name, process_specification, results_file_path, save_schema=True)
    if schema is None:
        raise ValueError("Stage 1 schema extraction failed. Cannot continue to Stage 2.")
    ...
    results_file_path = "../results/test/stage-2/Atomic-Layer-Deposition/experimental-schema"
    ...
    if not scientific_paper:
        raise ValueError("PDF text extraction failed for scientific document. Cannot continue to stage 2 of schema extraction.")
    schema = extract_schema_stage2(llm_model_name, schema, expert_review, scientific_paper, results_file_path, save_schema=True)
    if schema is None:
        raise ValueError("Stage 2 schema extraction failed. Cannot continue to Stage 3.")
    ...
    results_file_path = "../results/test/stage-3/Atomic-Layer-Deposition/experimental-schema"
    ...
    if not scientific_paper:
        raise ValueError("PDF text extraction failed for scientific document. Cannot continue to stage 3 of schema extraction.")
